...
dep_datasets <- list(
    ...
    Mageck_DRIVE = list(mod_type = 'mageck', dataset = 'DRIVE', type = 'abs',
                        taiga_name = 'mageck-d400', taiga_version = 1),
...
for (cur_dset in names(dep_datasets)) {
    if (dep_datasets[[cur_dset]]$type == 'abs') {
         if (normalization == 'global_z') {
          ...
          if (normalize_D2_per_CL & dep_datasets[[cur_dset]]$mod_type == 'D2') {
            cur_pos_set <- as.character(hart_ess[hart_ess %in% colnames(dep_data[[cur_dset]]$gene_scores)])
            cur_neg_set <- as.character(hart_non_ess[hart_non_ess %in% colnames(dep_data[[cur_dset]]$gene_scores)])
            cur_pos_medians <- apply(dep_data[[cur_dset]]$gene_scores[, cur_pos_set], 1, median, na.rm=T)
            cur_neg_medians <- apply(dep_data[[cur_dset]]$gene_scores[, cur_neg_set], 1, median, na.rm=T)
            cur_scale_facs <- pmax(cur_neg_medians - cur_pos_medians, 0.001)
            if (!is.null(dep_data[[cur_dset]]$gene_score_SDs)) {
                dep_data[[cur_dset]]$gene_score_SDs %<>% t() %>% scale(center = FALSE, scale = cur_scale_facs) %>% t()
            }
            dep_data[[cur_dset]]$gene_scores %<>% t() %>% scale(center = cur_neg_medians, scale = cur_scale_facs) %>% t()
          } else {
...
#per-CL scale normalization for Mageck scores
...
ggsave(file.path(fig_dir, 'CRISPR_cor_scatter.png'), width = 3.5, height = 3, dpi = 350)
```
...
print('Median D2 vs D1 improvement (%)')
with(bench_cors, median((abs(D2_DRIVE) - abs(D1_DRIVE))/abs(D1_DRIVE), na.rm=TRUE))
...
ggsave(file.path(fig_dir, 'bench_strong_cors_bar_Ach.png'), plot = p1, width = 3.5, height = 3.5, dpi = 350)
ggsave(file.path(fig_dir, 'bench_strong_cors_bar_DRIVE.png'), plot = p2, width = 3.5, height = 3.5, dpi = 350)
...
lineage_colors <- rainbow(length(lineage_list))
# par(mar=c(1,1,1,1))
...
# dep_data$ATARiS_merge$gene_scores[is.na(dep_data$ATARiS_merge$gene_scores)] <- NA
# D1_pcs <- pcaMethods::pca(dep_data$ATARiS_merge$gene_scores,
#                           method = 'ppca',
#                           nPcs = 2,
#                           scale = 'none',
#                           center = center)
...
a_df <- data.frame(CCLE_ID = rownames(dep_data$D2_comb$gene_scores), 
    ...
    # left_join(data.frame(
    #   CCLE_ID = rownames(dep_data$ATARiS_merge$gene_scores),
    #                D1_PC1 = D1_pcs[,1],
    #                D1_PC2 = D1_pcs[,2]
    # ), by = 'CCLE_ID') %>%
...
# D1_PC1_var <- D1_R2sum[1]*100
# D1_PC2_var <- (D1_R2sum[2]-D1_R2sum[1])*100
# p3 <- ggplot(a_df, aes(D1_PC1, D1_PC2, fill = CL_m)) +
#     geom_point(shape = 21, size = 2, color = 'white', alpha = 0.6) +
#     guides(fill = guide_legend(title = 'Batch')) +
#     xlab(sprintf('PC1 (%.1f%% variance)', D1_PC1_var)) +
#     ylab(sprintf('PC2 (%.1f%% variance)', D1_PC2_var)) +
#     ggtitle('D1-combined') +
#     # scale_fill_Publication() +
#     scale_fill_manual(values = cmap) +
#     theme_Publication(base_font)
...
```{r}
merge_sets <- list(RSA = list(Ach = 'RSA_Ach', DRIVE = 'RSA_DRIVE'),
...
CRISPR_cors %<>% left_join(comb_gene_avgs %>% 
                               filter(dset == 'D2_comb') %>% 
...
ggplot(CRISPR_cors %>% filter(dset %in% c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'D1_merge')) %>% 
...
<!-- ```{r} -->
<!-- #test introducing SD offset -->
<!-- SD_offset <- 0.05 -->
<!-- dep_data$D2_comb2 <- dep_data$D2_comb -->
<!-- dep_data$D2_comb2$gene_score_SDs <- sqrt(dep_data$D2_comb$gene_score_SDs^2 + SD_offset^2) -->
...
ggsave(file.path(fig_dir, 'combined_scatter_p.png'), width = 3.5, height = 3, dpi = 350)
...
# rel_an %<>% distinct(dset, targ_gene, feat_type, .keep_all = TRUE)
...
sum_stats %<>% mutate(MDP = factor(MDP, levels = MDP_plot_order))
...
ggsave(file.path(fig_dir, 'combined_top_class_tot.png'), width = 3.5, height = 3., dpi = 350)
...
### Detailed analysis of biomarkers
...
cur_res <- adply(all_cyclops, 1, function(row) {
  ...
  mutate(D1_best = pmax(D1_Ach, D1_DRIVE, na.rm=T),
  targ_symbol = cdsr::map_genes(targ_gene, 'entrez', 'hgnc', unique = FALSE)$mapping$hgnc) %>%
  mutate(is_Ach_CYCLOPS = targ_symbol %in% Tsherniak_CYCLOPS,
         is_DRIVE_CYCLOPS = targ_symbol %in% DRIVE_CYCLOPS) %>% 
  mutate(D2_imp = D2 - D1_best,
...
ggplot(D2_unique_rel_an, aes(avg_score, abs(cor), color = MDP)) + 
  geom_point() +
  ...
  scale_color_manual(values = cur_cols)
export_fig('new_MDP_scatter', width = 3.5, height = 3.5)
```
...
<!-- cur_gene_set <- NULL -->
<!-- cur_CL_set <- NULL -->
<!-- bench_res <- get_target_biomarker_dep_cors(target_dsets, curated_benchmarks, feature_data, cur_gene_set, cur_CL_set, use_bayes = use_bayes) -->
...
Marc_CL_batch_data <-read.csv(file.path(dep_datasets$D2_Marc$path, 'CL_batch_data.csv'), stringsAsFactors = F) 
...
ggsave(file.path(fig_dir, 'D2_corrs_smooth.png'), plot = p, width = 6.5, height = 3., dpi = 350)
...
ggsave(file.path(fig_dir, 'GAnorm_benchmark_ecdf.png'), width = 3.5, height = 3.25, dpi = 350)
...
BGPD.LFC.mat <- load.from.taiga(data.name='drive-lfc-matrices-3867', data.version=11, data.file='BGPD_LFC_mat')
poolA.LFC.mat <- load.from.taiga(data.name='drive-lfc-matrices-3867', data.version=11, data.file='poolA_LFC_mat')
poolB.LFC.mat <- load.from.taiga(data.name='drive-lfc-matrices-3867', data.version=11, data.file='poolB_LFC_mat')
...
ggplot(hp_ssmds_common, aes(Achilles, DRIVE)) + 
    ...
    theme_Publication() + 
    ggtitle('shRNA')
ggsave(file.path(fig_dir, 'LFC_SSMD_compare_common_shRNAs.png'), width = 3.5, height = 3.5, dpi = 350)
...
ggplot(df, aes(dep, fill = dset)) + 
    ...
    scale_fill_manual(values = data_type_pal) +
    theme_Publication()
ggsave(file.path(fig_dir, 'Ach_DRIVE_gene_SD_compare.png'), width = 3.5, height = 3, dpi = 350)
...
<!-- ```{r} -->
<!-- #analyze calibration of uncertainty estimates using distribution of t-stats for negative control genes -->
<!-- ncons <- as.character(hart_non_ess) -->
<!-- cur_SDs <- dep_data$D2_Ach$gene_score_SDs[, ncons[ncons %in% colnames(dep_data$D2_Ach$gene_score_SDs)]] -->
<!-- cur_means <- dep_data$D2_Ach$gene_scores[, ncons[ncons %in% colnames(dep_data$D2_Ach$gene_scores)]] -->
<!-- cur_t_stats <- cur_means/cur_SDs -->
<!-- plot(Ach_CL_data$gene_slope, apply(cur_t_stats, 1, sd, na.rm=T)) -->
<!-- ``` -->



<!-- ```{r} -->
<!-- #merge gene dep matrices -->
<!-- GA_qnorm <- merge(dep_data$GA_Ach$gene_scores, dep_data$GA_DRIVE$gene_scores, all = TRUE) %>%  -->
<!--   merge(dep_data$GA_Marc$gene_scores, all = TRUE) -->
<!-- GA_qnorm <- normalizeQuantiles(t(GA_qnorm)) %>% t() -->

<!-- CL_names <- c(rownames(dep_data$GA_Ach$gene_scores), -->
<!--               rownames(dep_data$GA_DRIVE$gene_scores), -->
<!--               rownames(dep_data$GA_Marc$gene_scores)) -->
<!-- unique_CLs <- unique(CL_names) -->
<!-- GA_qnorm_mat <- matrix(NA, nrow = length(unique_CLs), ncol = ncol(GA_qnorm)) -->
<!-- rownames(GA_qnorm_mat) <- unique_CLs -->
<!-- colnames(GA_qnorm_mat) <- colnames(GA_qnorm) -->
<!-- for (cur_CL in unique_CLs) { -->
<!--   GA_qnorm_mat[cur_CL,] <- colMeans(GA_qnorm[CL_names == cur_CL,, drop=F], na.rm=T)     -->
<!-- } -->

<!-- GA_qnorm_mat[is.na(GA_qnorm_mat)] <- NA -->
<!-- qGA_pcs <- pcaMethods::pca(GA_qnorm_mat, -->
<!--                           method = 'ppca', -->
<!--                           nPCs = 2, -->
<!--                           scale = 'none', -->
<!--                           center = center) -->
<!-- qGA_R2sum <- pcaMethods::R2cum(qGA_pcs) -->
<!-- qGA_pcs <- pcaMethods::scores(qGA_pcs) -->
...
imp_mat <- pcaMethods::completeObs(D1_pcs)
first_PC_D1_Ach <- pcaMethods::scores(D1_pcs)[,1] #first PC
first_PC_D1_Ach <- matrix(first_PC_D1_Ach/sqrt(sum(first_PC_D1_Ach^2, na.rm=T)), ncol = 1)
...
target_deps <- c('D1_Ach', 'D1PC_Ach', 'D1_DRIVE', 'D1PC_DRIVE',
                       'GA_Ach', 'GAPN_Ach', 'GA_DRIVE', 'GAPN_DRIVE','D2_Ach', 'D2_DRIVE')
...
```{r}
# target_dsets <- c('D1_Ach', 'D1PC_Ach', 'D2_Ach', 'D1_DRIVE', 'D1PC_DRIVE', 'D2_DRIVE', 'GA_Ach', 'GAPN_Ach', 'GA_DRIVE', 'GAPN_DRIVE')
...
# bench_res_test_spread <- spread(bench_res_test %>% dplyr::select(cor, model, Dep_Gene_ID) %>% mutate(cor = abs(cor)) %>% distinct(model, Dep_Gene_ID, .keep_all=T), model, cor)
