...
# fig_dir <- '~/Desktop/D2_figures_test'
...
target_dsets <- c('D1_DRIVE', 'D2_DRIVE', 'GA_DRIVE', 'D1_Ach', 'D2_Ach', 'GA_Ach', 'Mageck_Ach')
...
ggsave(file.path(fig_dir, 'PCA_compare.png'), plot = p, width = 8.5, height = 3, dpi = 350)
ggsave(file.path(fig_dir, 'PCA_compare.pdf'), plot = p, width = 8.5, height = 3, device = cairo_pdf())
...
```{r, fig.width=5, fig.height=4}
dset_map <- c(D2_Ach = 'D2 Ach', D2_DRIVE = 'D2 DRIVE', D2_comb = 'D2 comb', GA_comb = 'GA comb', CERES_CRISPR = 'CRISPR', D2_comb2 = 'D2 comb2')
ggplot(CRISPR_cors %>% filter(dset %in% c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'D2_comb2')) %>% 
...
# cur_prec <- 1/dep_data$D2_comb2$gene_score_SDs^2
# in_DRIVE_CLs <- rownames(cur_prec) %in% DRIVE_CLs
# cur_prec[in_DRIVE_CLs,] <- scale(cur_prec[in_DRIVE_CLs,], center = FALSE, scale = colSums(cur_prec[in_DRIVE_CLs,], na.rm=T))
# cur_prec[!in_DRIVE_CLs,] <- scale(cur_prec[!in_DRIVE_CLs,], center = FALSE, scale = colSums(cur_prec[!in_DRIVE_CLs,], na.rm=T))
# dep_data$D2_comb2$gene_score_SDs <- sqrt(1/cur_prec)
dep_data$D2_comb2$gene_score_SDs <- sqrt(dep_data$D2_comb$gene_score_SDs^2 + SD_offset^2)
# dep_data$D2_comb2$gene_score_SDs <- matrix(1, nrow = nrow(dep_data$D2_comb$gene_scores), ncol = ncol(dep_data$D2_comb$gene_scores)) %>%
#   set_colnames(colnames(dep_data$D2_comb$gene_scores)) %>%
  # set_rownames(rownames(dep_data$D2_comb$gene_scores))

```


```{r, include = FALSE}
target_dsets <- c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'D2_comb2')
...
ggsave(file.path(fig_dir, 'combined_bench_qvalues.png'), width = 3.5, height = 3.25, dpi = 350)
...
D2_unique_rel_an <- rel_an %>% 
  filter(!is.na(MDP)) %>% 
  filter(targ_gene %in% D2_unique, dset == 'D2_comb') %>% 
...
bench_res_test_spread <- spread(bench_res_test %>% dplyr::select(cor, model, Dep_Gene_ID) %>% mutate(cor = abs(cor)) %>% 
                                  distinct(model, Dep_Gene_ID, .keep_all=T), model, cor)

print('Wilcox test D1 Ach vs PCN')
with(bench_res_test_spread, wilcox.test(D1_Ach, D1PC_Ach, paired = TRUE))
with(bench_res_test_spread, median(D1_Ach - D1PC_Ach, na.rm=T))
print('N_used')
with(bench_res_test_spread, sum(!is.na(D1PC_Ach) & !is.na(D1_Ach)))

print('Wilcox test D1 DRIVE vs PCN')
with(bench_res_test_spread, wilcox.test(D1_DRIVE, D1PC_DRIVE, paired = TRUE))
with(bench_res_test_spread, median(D1_DRIVE - D1PC_DRIVE, na.rm=T))
print('N_used')
with(bench_res_test_spread, sum(!is.na(D1PC_DRIVE) & !is.na(D1_DRIVE)))
