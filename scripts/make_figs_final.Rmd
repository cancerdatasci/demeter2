...
fig_dir <- '~/CPDS/demeter2/results/figures_lowreg'
...
dep_datasets <- list(
    D2_DRIVE = list(mod_type = 'D2', path = file.path(base_data_dir, 'kube_results/DRIVE_final_prev/1'), dataset = 'DRIVE', type = 'abs'),
    D2_Ach = list(mod_type = 'D2', path = file.path(base_data_dir, 'kube_results/Ach_lowreg/1'), dataset = 'Ach', type = 'abs'),
    D2_Marc = list(mod_type = 'D2', path = file.path(base_data_dir, 'kube_results/Marc_final/1'), dataset = 'Marcotte', type = 'abs'),
    D2_comb = list(mod_type = 'D2', path = file.path(base_data_dir, 'kube_results/comb_final_prev/1'), dataset = 'DRIVE', type = 'abs'),
...
Ach_CL_data %<>% mutate(CCLE_ID = plyr::revalue(CCLE_ID, new_name_map))

DRIVE_CL_data <- read.csv(file.path(dep_datasets$D2_DRIVE$path, 'CL_data.csv'), stringsAsFactors = F, check.names = F)
DRIVE_CL_data %<>% mutate(CCLE_ID = plyr::revalue(CCLE_ID, new_name_map))
...
(temp['total', 'D2 comb'] / temp['total', 'D1 Ach']) 
(temp['total', 'D2 comb']) / (temp['total', 'D1 DRIVE'])
(temp['total', 'D2 comb'] / temp['total', 'GA Ach']) 
(temp['total', 'D2 comb']) / (temp['total', 'GA DRIVE'])
