...
recompute_biomarker <- TRUE
biomarker_res_cache_GE <- '~/CPDS/demeter2/results/compare_biomarker_cache_GE.rds'
biomarker_res_cache_CN <- '~/CPDS/demeter2/results/compare_biomarker_cache_CN.rds'
biomarker_res_cache_DAM_MUT <- '~/CPDS/demeter2/results/compare_biomarker_cache_DAM_MUT.rds' 
biomarker_res_cache_HOT_MUT <- '~/CPDS/demeter2/results/compare_biomarker_cache_HOT_MUT.rds' 
...
related_genes <- load.from.taiga(data.name='related-features-dcbd', data.version=2)
...
dep_data <- load_all_dep_data(dep_dnames, dep_datasets, include_gene_families = include_gene_families, use_bayes = use_bayes)
...
```{r, include = FALSE}
#make consistent naming for CLs in Marcotte dataset
Marc_CLs <- str_match(rownames(dep_data$D2_Marc$gene_scores), '(.+)_')[,2]
rownames(dep_data$D1_merge$gene_scores)[rownames(dep_data$D1_merge$gene_scores) %in% Marc_CLs] <- paste0(rownames(dep_data$D1_merge$gene_scores)[rownames(dep_data$D1_merge$gene_scores) %in% Marc_CLs], '_BREAST')
rownames(dep_data$GA_comb$gene_scores)[rownames(dep_data$GA_comb$gene_scores) %in% Marc_CLs] <- paste0(rownames(dep_data$GA_comb$gene_scores)[rownames(dep_data$GA_comb$gene_scores) %in% Marc_CLs], '_BREAST')
```
...
with(df, cor.test(AGO2_GE, Ach_GA_ssmd))
with(df, cor.test(AGO2_GE, DRIVE_GA_ssmd))
...
g1 <- ggplot(to_plot, aes(CN, D1, fill = `Screen signal`)) + 
    ...
    scale_fill_gradient2(midpoint = 1, limits = c(0.5, 2), oob = squish, breaks = c(0.5, 2)) +
...
with(to_plot, cor.test(`Screen signal`, D1, use = 'pairwise.complete.obs'))
with(to_plot, cor.test(CN, D1, use = 'pairwise.complete.obs'))
...
g2 <- ggplot(to_plot, aes(CN, D2, fill = `Screen signal`)) + 
    ...
    # geom_smooth(method = 'lm', color = 'black') +
    # guides(color = guide_legend(title = 'Screen signal')) +
   scale_fill_gradient2(midpoint = 1, limits = c(0.5, 2), oob = squish) +
    xlab(paste0(Gene_ID_to_symb[examp_CYCLOPS], ' relative CN (log2)')) +
...
with(to_plot, cor.test(`Screen signal`, D2, use = 'pairwise.complete.obs'))
with(to_plot, cor.test(CN, D2, use = 'pairwise.complete.obs'))
...
unc_df <- per_CL_stats %>% 
    ...
    left_join(read.csv('~/CPDS/demeter2/results/Ach_rep_cors.csv', stringsAsFactors = F, check.names = F), by = "CCLE_ID")
...
max_comb <- max(-log10(cur_bench_p$D2_comb), na.rm=T)
...
with(bench_p, wilcox.test(-log10(D2_comb), -log10(D2_Ach), paired = TRUE))
with(bench_p, wilcox.test(-log10(D2_comb), -log10(D2_DRIVE), paired = TRUE))
...
include_superpath <- FALSE
 targ_dsets <- c('D2_comb', 'D2_DRIVE', 'D1_DRIVE', 'GA_Ach', 'GA_DRIVE', 'D1_Ach', 'D2_Ach', 'CERES_CRISPR')
...
(temp['total', 'D2 comb'] - temp['total', 'D2 DRIVE']) / (temp['total', 'D2 DRIVE'])
(temp['total', 'D2 comb'] - temp['total', 'D2 Ach']) / (temp['total', 'D2 Ach'])
(temp['total', 'D2 comb'] - temp['total', 'D1 Ach']) / (temp['total', 'D1 Ach'])
(temp['total', 'D2 comb'] - temp['total', 'GA Ach']) / (temp['total', 'GA Ach'])
(temp['total', 'D2 comb']) / (temp['total', 'GA Ach'])
...
rel_an %<>% mutate(MDP = NA,
  ...
  MDP = ifelse(source %in% c('CORUM', 'ppi'), "physical interaction", MDP)
)
...
Marc_CL_data <-read.csv(file.path(dep_datasets$D2_Marc$path, 'CL_data.csv'), stringsAsFactors = F)
Marc_CL_data %<>% mutate(CCLE_ID = clean_CL_names_with_missing(CCLE_ID))
...
with(temp_per_CL_stats, wilcox.test(GA_Ach, GA_DRIVE))
