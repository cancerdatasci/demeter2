...
normalize_D2_per_CL <- FALSE # experimental, should D2 data be pos/neg normed per CL
...
#parameters controlling (expensive) computation of all pairwise dep-feature associations
...
dep_datasets <- list(
    ...
    RSA_DRIVE = list(mod_type = 'RSA', dataset = 'DRIVE', type = 'abs',
                   ...
                   taiga_version = 12),
    MAGeCK_Ach = list(mod_type = 'mageck', dataset = 'Ach', type = 'abs',
                      taiga_name = 'achilles-mageck-scores-4f94',
                      taiga_version = 2),
    MAGeCK_DRIVE = list(mod_type = 'mageck', dataset = 'DRIVE', type = 'abs',
                        taiga_name = 'mageck-d400', taiga_version = 4),
...
#load dataset of gene-gene relationships
...
#load pre-computed CRISPR dep-feature relationships
...
dep_data <- load_all_dep_data(dep_dnames, 
                              dep_datasets, 
                              include_gene_families = include_gene_families, 
                              use_bayes = use_bayes)
#convert these gene names to Entrez IDs if needed
...
```{r}
#run some checks on the dep data loading
sample_info <- read.csv('~/CPDS/data/D2_figshare/sample_info.csv', stringsAsFactors = F, check.names = F)
for (dep_name in setdiff(names(dep_data), 'CERES_CRISPR')) {
 #make sure all cell line names are in the sample info file
 stopifnot(all(rownames(dep_data[[dep_name]]$gene_scores) %in% sample_info$CCLE_ID))

  #make sure all gene names are entrez ids
all(grepl('[0-9]', colnames(dep_data[[dep_name]]$gene_scores)))
is.character(colnames(dep_data[[dep_name]]$gene_scores))
}
...
sprintf('Ach data, %d hps targeting %d genes, with %f hps per gene avg. %f median',
        nrow(Ach_hp_data), 
        nrow(Ach_hp_by_gene),
        mean(Ach_hp_by_gene$n), 
        median(Ach_hp_by_gene$n))
...
DR_hp_by_gene <- DR_hp_data %>% 
    ...
    summarise(n = n(), max_Geff = max(Geff, na.rm=T)) %>% 
    filter(n >= min_hps_per_gene)
sprintf('DRIVE data, %d hps targeting %d genes, with %f hps per gene avg. %f median',
        nrow(DR_hp_data), 
        nrow(DR_hp_by_gene),
        mean(DR_hp_by_gene$n), 
        median(DR_hp_by_gene$n))
```


```{r}
#extract hp sets used by D2 models
...
tot_hp_set <- Ach_hp_set %>% 
  union(DRIVE_hp_set) %>% 
  union(Marc_hp_set)
sprintf('%d total hairpins', length(tot_hp_set))
```
...
for (cur_dset in target_dsets) {
    #calculate shRNA efficacy stats per gene 
    cur_avg_Geff <- read.csv(file.path(dep_data[[cur_dset]]$path, 'hp_data.csv'), 
    ...
    bad_genes <- cur_avg_Geff %>% 
        ...
        .[['Gene_ID']]
    print(sprintf('%s: removing %d/%d bad genes', 
                  cur_dset, 
                  length(bad_genes), 
                  ncol(dep_data[[cur_dset]]$gene_scores)))
...
curated_benchmarks <- read.csv('~/CPDS/data/general/benchmark_list_cleaned.csv', stringsAsFactors = F, check.names=F) %>% 
    ...
    dplyr::distinct(Feat_gene, Dep_Gene, feat_type, .keep_all=T)
...
CRISPR_benchmarks %<>% 
    filter(abs(cor) > bench_cor_thresh) %>% #only keep sufficiently strong feature-dep corrs
    group_by(feat_type) %>% 
    top_n(n = n_bench_biomarkers_per_type, wt = abs(cor)) %>% #take top n by feature type
    ungroup() 
...
# #per-CL scale normalization for MAGeCK scores (before applying global pos/neg norm)
# if ('MAGeCK_Ach' %in% names(dep_data)) {
#   dep_data$MAGeCK_Ach$gene_scores %<>%
#     t() %>%
#     scale(center = T, scale = T) %>%
#     t()
# }
# if ('MAGeCK_DRIVE' %in% names(dep_data)) {
#    dep_data$MAGeCK_DRIVE$gene_scores %<>%
#     t() %>%
#     scale(center = T, scale = T) %>%
#     t()
# }
...
for (cur_dset in names(dep_datasets)) {
    if (dep_datasets[[cur_dset]]$type == 'abs') {
         if (normalization == 'global_z') {
            dep_data[[cur_dset]] <- normalize_genescores_global_z(dep_data[[cur_dset]])
        } else if (normalization == 'pos_neg') {
            dep_data[[cur_dset]] <- normalize_genescores_posneg(
                                        dep_data[[cur_dset]], 
                                        hart_ess, 
                                        hart_non_ess, 
                                        cur_gene_avgs %>% filter(dset == cur_dset),
                                        normalize_D2_per_CL)
        }         
    } else {
...
gene_avgs <- get_gene_avgs(dep_data, names(dep_data), hart_ess, hart_non_ess, use_bayes = use_bayes) %>% 
    mutate(Gene = as.character(Gene))

gene_SDs <- get_gene_SDs(dep_data, names(dep_data), hart_ess, hart_non_ess, use_bayes = use_bayes) %>% 
    mutate(Gene = as.character(Gene))
...
ov_gene_sep_stats <- get_gene_avg_stats(dep_data, gene_avgs, names(dep_data), only_overlapping_genes, use_bayes = use_bayes)
...
per_CL_stats <- get_per_CL_stats(dep_data, names(dep_data), only_overlapping_genes, neg_control_type, feature_data$GE, hart_ess, hart_non_ess, use_bayes = use_bayes)

```


```{r, include = FALSE}
# MAKE manual merge of 3 D1 datasets 
...
## S1: Distribution of SSMDs per cell line

```{r}
p1 <- ggplot(per_CL_stats %>% 
           filter(dset %in% c('D2_Ach', 'GA_Ach', 'RSA_Ach', 'MAGeCK_Ach')) %>% 
...
p2 <- ggplot(per_CL_stats %>% 
           filter(dset %in% c('D2_DRIVE', 'GA_DRIVE', 'RSA_DRIVE', 'MAGeCK_DRIVE')) %>% 
           mutate(model = str_match(dset, '(.+)_')[,2]),
       aes(ssmd, fill = model)) + 
    geom_density(alpha = 0.6) + 
    scale_fill_manual(values = mod_type_pal) +
...
df <- per_CL_stats %>% 
    dplyr::select(CCLE_ID, dset, ssmd) %>% 
    spread(dset, ssmd) %>% 
    mutate(D2_Ach_GA_imp = (D2_Ach - GA_Ach)/GA_Ach*100,
...
print(sprintf('Achilles D2 avg percent improvement over GA: %.3f across %d/%d cell lines', mean(df$D2_Ach_GA_imp, na.rm=T), sum(df$D2_Ach_GA_imp > 0, na.rm=T), sum(!is.na(df$D2_Ach_GA_imp))))
print(sprintf('Achilles D2 avg percent improvement over RSA: %.3f across %d/%d cell lines', mean(df$D2_Ach_RSA_imp, na.rm=T), sum(df$D2_Ach_RSA_imp > 0, na.rm=T), sum(!is.na(df$D2_Ach_RSA_imp))))
print(sprintf('DRIVE D2 avg percent improvement over GA: %.3f across %d/%d cell lines', mean(df$D2_DRIVE_GA_imp, na.rm=T), sum(df$D2_DRIVE_GA_imp > 0, na.rm=T), sum(!is.na(df$D2_DRIVE_GA_imp))))
print(sprintf('DRIVE D2 avg percent improvement over RSA: %.3f across %d/%d cell lines', mean(df$D2_DRIVE_RSA_imp, na.rm=T), sum(df$D2_DRIVE_RSA_imp > 0, na.rm=T), sum(!is.na(df$D2_DRIVE_RSA_imp))))

```


## 1C: Per-gene avg SSMD across datasets
...
## 1c: Per-gene avg agreement across datasets
...
Ach_DRIVE_MAGeCK <- cor(cur_gene_avgs$MAGeCK_Ach, cur_gene_avgs$MAGeCK_DRIVE, use = 'pairwise.complete.obs')

df <- data.frame(CERES_cors) %>% 
    rownames_to_column(var = 'dset') %>% 
    set_colnames(c('dset', 'cor')) %>% 
    filter(grepl('D2', dset) | grepl('GA', dset) | grepl('RSA', dset) | grepl('MAGeCK', dset)) %>% 
  ...
  rbind(data.frame(dset = 'MAGeCK_Ach_DRIVE', cor = Ach_DRIVE_MAGeCK))
...
## 2b: SSMD correlates with AGO2 GE
...
ggplot(df, aes(AGO2_GE)) + 
    ylab('Screen signal') +
    xlab('AGO2 Gene expression (log RPKM)') +
...
ggplot(df, aes(AGO2_GE)) + 
    ...
    scale_fill_manual(values = data_type_pal) +
    theme_Publication(base_font)
ggsave(file.path(fig_dir, 'AGO2_vs_SSMD.png'), width = 3.6, height = 3.25, dpi = 350)

print('AGO2-SSMD cor Achilles')
print(with(df, cor.test(AGO2_GE, Ach_GA_ssmd, method = 'spearman')))
#if we exclude the two outliers CLs
print(with(df %>% filter(AGO2_GE > 0), cor.test(AGO2_GE, Ach_GA_ssmd, method = 'spearman')))
...
AGO2_cors %<>% mutate(gene_type = plyr::revalue(gene_type, c(non_essential = 'non-essential')))
...
## S4: Removal of AGO2 GE correlations for Achilles and DRIVE datasets

```{r, fig.width=5, fig.height=4}
AGO2_cors %<>% mutate(mod = str_match(model, '([:alnum:]+)_[:alnum:]+')[,2],
                      dat = str_match(model, '[:alnum:]+_([:alnum:]+)')[,2],
                      mod = factor(mod, levels = c('D2', 'D1', 'ATARiS', 'GA', 'RSA', 'MAGeCK')))
...
dep_cor_res <- get_dep_corrs(dep_data, target_dsets, 
                             high_var_genes, 
                             gene_avgs, 
                             use_bayes)
```

## 3a-b Correlation vs avg essentiality heatmaps
...
## S6: Corr-vs-essentiality smoothed plots
...
## 3c-d Example corrnet fig

```{r}
q_gene <- 'MED14'
q_gene_ID <- Gene_symb_to_ID[q_gene]
#get related gene IDs (protein complex co-members)
...
## 4a: D2 improves estimates of dependency profiles
...
p1 <- ggplot(CRISPR_cors %>% filter(grepl('Ach', dset)) %>% 
           mutate(model = str_match(dset, '([:alnum:]+)_')[,2],
                  model = factor(model, levels = c('D2', 'D1', 'ATARiS', 'GA', 'RSA', 'MAGeCK'))), 
...
ggsave(file.path(fig_dir, 'CRISPR_cor_smooth_Ach.png'), plot = p1, width = 3.5, height = 3, dpi = 350)
ggsave(file.path(fig_dir, 'CRISPR_cor_smooth_DRIVE.png'), plot = p2, width = 3.5, height = 3, dpi = 350)

```

## 4b: Correlations with gene expression
...
p1 <- ggplot(GE_cors %>% filter(grepl('Ach', dset)) %>% 
    ...
    xlab('Cell line avg. dependency score') +
    coord_cartesian(xlim = c(-1.5, 0.5), ylim = c(0, 0.35)) +
    guides(color = guide_legend(title = '', override.aes = list(lwd = 3))) +
    scale_color_manual(values = mod_type_pal) +
    theme_Publication(base_font)
...
p2 <- ggplot(GE_cors %>% filter(grepl('DRIVE', dset)) %>% 
    ...
    ylab('Avg. dep-GE\ncorrelation magnitude') +
    xlab('Cell line avg. dependency score') +
    coord_cartesian(xlim = c(-1.5, 0.5), ylim = c(0, 0.375)) +
    guides(color = guide_legend(title = '', override.aes = list(lwd = 3))) +
    scale_color_manual(values = mod_type_pal) +
    theme_Publication(base_font)
...
ggsave(file.path(fig_dir, 'GE_cor_smooth_Ach.png'), plot = p1, width = 3.5, height = 3, dpi = 350)
ggsave(file.path(fig_dir, 'GE_cor_smooth_DRIVE.png'), plot = p2, width = 3.5, height = 3, dpi = 350)

```

## 4c: Correlations with copy number
...
ggsave(file.path(fig_dir, 'CN_cor_smooth_Ach.png'), plot = p1, width = 3.5, height = 3, dpi = 350)
ggsave(file.path(fig_dir, 'CN_cor_smooth_DRIVE.png'), plot = p2, width = 3.5, height = 3, dpi = 350)
...
ggsave(file.path(fig_dir, 'cyclops_example.png'), plot = p, width = 6., height = 3.5, dpi = 350)
...
## 4e: Benchmark feature-dep corrs

```{r}
target_dsets <- names(dep_data)
cur_gene_set <- NULL
cur_CL_set <- NULL
...
```{r}
## PRINT STATS
print('Achilles STATS')
print_paired_two_group_stats(abs(bench_cors$D2_Ach), abs(bench_cors$D1_Ach))

print('DRIVE STATS')
print_paired_two_group_stats(abs(bench_cors$D2_DRIVE), abs(bench_cors$D1_DRIVE))
```
...
p1 <- ggplot(df %>% filter(grepl('Ach', variable)) %>% 
    ...
    ylab('CDF') +
    coord_cartesian(xlim = c(0.6, 0), ylim = c(0, 0.75)) +
    # xlim(0.6, 0) +
    # ylim(0, 0.75) +
...
p2 <- ggplot(df %>% filter(grepl('DRIVE', variable)) %>% 
    ...
    xlab('Feature-dep. corr. magnitude') +
    ylab('CDF') +
    coord_cartesian(xlim = c(0.75, 0), ylim = c(0, 0.75)) +
    guides(color = guide_legend(title = '', override.aes = list(size = 3))) +
    geom_vline(xintercept = bench_cor_thresh, linetype = 'dashed') + 
    scale_color_manual(values = mod_type_pal) +
    theme_Publication(base_font)
...
ggsave(file.path(fig_dir, 'bench_ecdf_Ach.png'), plot = p1, width = 3.5, height = 3, dpi = 350)
ggsave(file.path(fig_dir, 'bench_ecdf_DRIVE.png'), plot = p2, width = 3.5, height = 3, dpi = 350)
...
ggsave(file.path(fig_dir, 'bench_strong_cors_bar_Ach.png'), plot = p1, width = 3.5, height = 3., dpi = 350)
ggsave(file.path(fig_dir, 'bench_strong_cors_bar_DRIVE.png'), plot = p2, width = 3.5, height = 3., dpi = 350)
...
```{r}
CL_meta_data <- read.csv('~/CPDS/data/D2_figshare/sample_info.csv', stringsAsFactors = F, check.names = F)

CL_meta_data %<>% mutate(
  disease = ifelse(is.na(disease), 'other', disease),
  disease = ifelse(disease == '', 'other', disease)
)

min_CLs <- 10
disease_df <- CL_meta_data %>% 
  group_by(disease) %>% 
  summarise(N_disease = n()) %>% 
  ungroup() %>% 
  mutate(disease = ifelse(N_disease < min_CLs, 'other', disease)) %>% 
  group_by(disease) %>% 
  summarise(N_disease = sum(N_disease)) %>% 
  ungroup() %>% 
  arrange(disease)

labels <- paste(disease_df$disease, ' (', disease_df$N_disease, ')')
lineage_colors <- rainbow(length(labels))
pdf(file.path(fig_dir, 'lineage_pie_chart.pdf'))
pie(disease_df$N_disease, col=lineage_colors, labels=labels, radius = 0.5, cex = 0.8, width = 4, height = 4, units = 'inches')
dev.off()
```
...
library(pcaMethods)
center <- TRUE #mean-center per gene 
...
D2_PC1_var <- D2_R2sum[1]*100
D2_PC2_var <- (D2_R2sum[2]-D2_R2sum[1])*100
p1 <- ggplot(a_df, aes(D2_PC1, D2_PC2, fill = CL_m)) + 
    ...
    xlab(sprintf('PC1 (%.1f%% variance)', D2_PC1_var)) + 
    ylab(sprintf('PC2 (%.1f%% variance)', D2_PC2_var)) + 
    ggtitle('D2-combined') +
    scale_fill_manual(values = cmap) +
    theme_Publication(base_font) 

GA_PC1_var <- GA_R2sum[1]*100
GA_PC2_var <- (GA_R2sum[2]-GA_R2sum[1])*100
p2 <- ggplot(a_df, aes(GA_PC1, GA_PC2, fill = CL_m)) + 
    geom_point(shape = 21, size = 2, color = 'white', alpha = 0.6) + 
    guides(fill = guide_legend(title = 'Batch')) +
    xlab(sprintf('PC1 (%.1f%% variance)', GA_PC1_var)) + 
    ylab(sprintf('PC2 (%.1f%% variance)', GA_PC2_var)) + 
    ggtitle('GA-combined') +
    scale_fill_manual(values = cmap) +
    theme_Publication(base_font) 
...
ggsave(file.path(fig_dir, 'PCA_compare.png'), plot = p, width = 6., height = 3, dpi = 350)
ggsave(file.path(fig_dir, 'PCA_compare.pdf'), plot = p, width = 6., height = 3, device = cairo_pdf())
...
```{r, include = FALSE}
## Calculate gene-level stats for combined datsets

target_dsets <- c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'CERES_CRISPR')

#get genes present in all target datasets
cur_gene_set <- Reduce(intersect, llply(dep_data[target_dsets], function(d) {colnames(d$gene_scores)}))

#genes present in RNAi datasets
comb_RNAi_common_genes <- Reduce(intersect, llply(dep_data[setdiff(target_dsets, 'CERES_CRISPR')], function(d) {colnames(d$gene_scores)}))
comb_RNAi_common_CLs <- Reduce(intersect, llply(dep_data[setdiff(target_dsets, 'CERES_CRISPR')], function(d) {rownames(d$gene_scores)}))

#calculate combined gene averages
comb_gene_avgs <- ldply(target_dsets, function(cur_dset) {
    if (cur_dset == 'CERES_CRISPR') {
      ...
      if (dep_datasets[[cur_dset]]$mod_type == 'D2' & use_bayes) {
              cur_gene_avgs <- sapply(cur_gene_set, function(cur_gene) {
                  wtd.mean(dep_data[[cur_dset]]$gene_scores[comb_RNAi_common_CLs, cur_gene],
                           1/dep_data[[cur_dset]]$gene_score_SDs[comb_RNAi_common_CLs, cur_gene]^2)
              })
        } else {
            cur_gene_avgs <- colMeans(dep_data[[cur_dset]]$gene_scores[comb_RNAi_common_CLs, cur_gene_set], na.rm=T)
        }
   }
...
comb_gene_avgs %<>% mutate(gene_type = get_gene_type(Gene, hart_ess, hart_non_ess))
comb_gene_avgs_wide <- spread(comb_gene_avgs, dset, avg_score)

ov_gene_sep_stats <- get_gene_avg_stats(dep_data, comb_gene_avgs, names(dep_datasets), only_overlapping_genes = FALSE, use_bayes = use_bayes)

```


## S9b: Gene-avg correlations with CRISPR 
...
CERES_cors <- cor(comb_gene_avgs_wide[, setdiff(target_dsets, 'CERES_CRISPR')], comb_gene_avgs_wide[, 'CERES_CRISPR', drop=F], use = 'pairwise.complete.obs')
df <- data.frame(CERES_cors) %>% 
    rownames_to_column(var = 'dset') %>% 
    set_colnames(c('dset', 'cor')) %>% 
...
map_names <- c(D2_Ach = 'D2 Ach', D2_DRIVE = 'D2 DRIVE', D2_comb = 'D2 comb', GA_comb = 'GA comb', D1_merge = 'D1 comb')
df %<>% mutate(dset = str_replace(dset, '_CRISPR', ''),
               dset = map_names[dset])
p2 <- ggplot(df %>% 
               mutate(dset = factor(dset, levels = df %>%
                       arrange(desc(cor)) %>% .[['dset']])), 
       aes(dset, cor, fill = model)) + 
    geom_bar(stat = 'identity') +
    guides(fill = FALSE) +
    ylab('Correlation of cell line\navg. with CRISPR') +
    scale_fill_manual(values = mod_type_pal) +
    theme_Publication(base_font) +
...
## S9c: Per-gene CRISPR correlations

```{r, include = FALSE}
target_dsets <- c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'CERES_CRISPR', 'D1_merge')
cur_CL_set <- intersect(comb_RNAi_common_CLs, rownames(dep_data$CERES_CRISPR$gene_scores))
...
CRISPR_cors_comb <- get_mat_dep_cors(setdiff(target_dsets, 'CERES_CRISPR'), target_mat, use_bayes)
CRISPR_cors_comb %<>% left_join(comb_gene_avgs %>% 
                               filter(dset == 'D2_comb') %>% 
...
```{r, fig.width=5, fig.height=4}
dset_map <- c(D2_Ach = 'D2 Ach', D2_DRIVE = 'D2 DRIVE', D2_comb = 'D2 comb', GA_comb = 'GA comb', CERES_CRISPR = 'CRISPR', D1_merge = 'D1 comb')

ggplot(CRISPR_cors_comb %>% filter(dset %in% c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'D1_merge')) %>% 
           mutate(dset = dset_map[dset]), 
       aes(avg_essentiality, cor, color = dset)) + 
    geom_smooth(lwd = 0.5) +
    xlab('Cell line avg. dependency score') +
    ylab('Avg. RNAi-CRISPR\ndependency corr') +
    guides(color = guide_legend(title = '', override.aes = list(lwd = 3), nrow = 2)) +
    coord_cartesian(xlim = c(-1.5, 0.5), ylim = c(0, 0.4)) +
    scale_color_manual(values = data_type_pal) +
    theme_Publication(base_font) +
    theme(legend.text = element_text(size = rel(0.7)))

CRISPR_cors_spread <- CRISPR_cors_comb %>% 
  dplyr::select(Gene, cor, dset) %>% 
  spread(dset, cor)

ggsave(file.path(fig_dir, 'combined_CRISPR_corr_smooth.png'), width = 3.5, height = 3, dpi = 350)

## STATS
print('D2 comb vs D2 Achilles')
print_paired_two_group_stats(CRISPR_cors_spread$D2_comb, CRISPR_cors_spread$D2_Ach)

print('D2 comb vs D2 DRIVE')
print_paired_two_group_stats(CRISPR_cors_spread$D2_comb, CRISPR_cors_spread$D2_DRIVE)
```


```{r, include = FALSE}
target_dsets <- c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb','D1_merge')
bench_res_overlap <- get_target_biomarker_dep_cors(target_dsets, 
                                                    CRISPR_benchmarks, 
                                                    feature_data, 
                                                   comb_RNAi_common_genes, 
                                                   comb_RNAi_common_CLs,
                                                   use_bayes = use_bayes)
...
## STATS
print('D2 comb vs D2 Ach')
print_paired_two_group_stats(abs(bench_cors_overlap$D2_comb),
                             abs(bench_cors_overlap$D2_Ach))
print('D2 comb vs D2 DRIVE')
print_paired_two_group_stats(abs(bench_cors_overlap$D2_comb),
                             abs(bench_cors_overlap$D2_DRIVE))
```
...
df <- bench_cors_overlap[, target_dsets] %>% 
  melt() %>% 
  mutate(variable = as.character(variable))
ggplot(df %>% filter(variable %in% c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'D1_merge')) %>% mutate(variable = map_names[variable],
                                                                                             variable = factor(variable, levels = c('D2 comb', 'D2 DRIVE', 'D2 Ach', 'GA comb', 'D1 comb'))), aes(abs(value), color = variable)) + 
    stat_ecdf(size = 0.6) +
    scale_x_reverse() +
    guides(color = guide_legend(title = '', override.aes = list(size = 3), nrow = 2)) +
    xlab('Feature-dep. corr. magnitude') +
    ylab('CDF') + 
    coord_cartesian(xlim = c(0.7, 0.0), ylim = c(0,1)) +
    # xlim(0.6, 0.2) +
    # ylim(0, 1) +
    geom_vline(xintercept = bench_cor_thresh, linetype = 'dashed') +
    scale_color_manual(values = data_type_pal) +
...
ggsave(file.path(fig_dir, 'combined_bench_ecdf.png'), width = 3.5, height = 3, dpi = 350)
```

### Barplot inset

```{r}
df <- bench_cors_overlap[, c(target_dsets)] %>% 
  melt() %>% 
  mutate(variable = as.character(variable))
df %<>% 
    group_by(variable) %>% 
...
##5c: Benchmark dep-feature corr p-vals for combined datasets

```{r, fig.width=5, fig.height=4.5}
cur_bench_p <- bench_res %>% 
    dplyr::select(-cor) %>% 
    spread(model, p.value) %>% filter(Dep_Gene_ID %in% comb_RNAi_common_genes)

df <- cur_bench_p[,  c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'D1_merge')] %>%
    melt() %>% 
...
ggplot(df %>% filter(dset %in% c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'D1_merge')) %>% 
           mutate(dset = as.character(dset),
                  dset = map_names[as.vector(dset)],
                  dset = factor(dset, levels = c('D2 comb', 'D2 DRIVE', 'D2 Ach', 'GA comb', "D1 comb"))), 
                     aes(-log10(p.value), color = dset)) + 
    stat_ecdf(size = 0.6) +
    scale_x_reverse() +
    coord_cartesian(xlim = c(80,0), ylim = c(0,1)) +
    guides(color = guide_legend(title = '', override.aes = list(size = 3), nrow = 2)) +
...
ggsave(file.path(fig_dir, 'combined_bench_qvalues.png'), width = 3.75, height = 3.5, dpi = 350)


print('p-vals D2_comb vs D2 Ach')
print_paired_two_group_stats(bench_p$D2_comb, bench_p$D2_Ach)

print('p-vals D2_comb vs D2 DRIVE')
print_paired_two_group_stats(bench_p$D2_comb, bench_p$D2_DRIVE)

```


## 5d-e: Improved identification of relevant biomarkers
...
## 5d: Number of features detected per dataset

```{r}
include_superpath <- FALSE

#load in all precomputed correlations
rel_an <- read_rds(biomarker_res_cache_GE) %>% mutate(feat_type = 'GE')
...
#keep top feature-dep correlation for each depenednecy/dset pair
...
ggsave(file.path(fig_dir, 'combined_N_biomarks_found.png'), width = 3.5, height = 3.5, dpi = 350)
...
#only count pairs once per relationship type
rel_an %<>% distinct(dset, feat_gene, targ_gene, MDP, .keep_all=T)

#single MDP per target feature pair
rel_an %<>% mutate(MDP_ord = plyr::revalue(MDP, 
                                 c(CYCLOPS = 5,
                                   `Onc mut` = 4,
                                   `Onc exp` = 3,
                                   `paralog loss` = 2,
                                   `interacting protein` = 1)))
rel_an %<>% group_by(dset, targ_gene, feat_gene) %>% 
  top_n(1, wt = MDP_ord) %>% 
  ungroup()


MDP_plot_order <- c('CYCLOPS', 'interacting protein', 'paralog loss', 'Onc exp', 'Onc mut')

#summary stats of MDPs detected per dataset
sum_stats <- rel_an %>%
    filter(!is.na(MDP)) %>% 
    group_by(dset, MDP) %>% 
    summarise(n = n()) %>% 
...
mod_order <- sum_stats %>% 
    ...
    .[['dset']]
sum_stats %<>% mutate(dset = factor(dset, levels = mod_order),
                      MDP = factor(MDP, levels = MDP_plot_order))
...
ggplot(sum_stats, 
    ...
    guides(fill = guide_legend(title = element_blank(), nrow = 3)) 

ggsave(file.path(fig_dir, 'combined_top_MDP_class.png'), width = 3.5, height = 3.5, dpi = 350)
...
##5f: Detailed analysis of biomarkers

```{r}
Tsherniak_supp <- read.csv('~/CPDS/data/Achilles/shRNA/Tsherniak_supp_table_MDP.csv', stringsAsFactors = F, check.names = F)
Tsherniak_CYCLOPS <- Tsherniak_supp %>%
  filter(`MDP feature group` == 'CYCLOPS') %>%
  .[['Gene dependency']]
DRIVE_CYCLOPS <- read_lines('~/CPDS/data/Novartis/DRIVE_CYCLOPS.txt')

# #extract Cyclops MDP pairs
# cyclops <- filter(rel_an, MDP == 'CYCLOPS', dset %in% c('D2_comb', 'D1_Ach', 'D1_DRIVE')) %>%
#   left_join(Tsherniak_supp, by = c('target' = 'Gene dependency'))
# all_cyclops <- cyclops %>%
#   distinct(targ_gene, feat_gene)
#
# #for each CYCLOPS gene, get best CN or GE dep-feature correlation for each dep dataset
# use_zscore <- FALSE
# cur_res <- adply(all_cyclops, 1, function(row) {
#   D1_Ach_cors <- get_best_assoc('D1_Ach', row$targ_gene, row$feat_gene, z_score = use_zscore)
#   D1_DRIVE_cors <- get_best_assoc('D1_DRIVE', row$targ_gene, row$feat_gene, z_score = use_zscore)
#   D2_comb_cors <- get_best_assoc('D2_comb', row$targ_gene, row$feat_gene, z_score = use_zscore)
#   return(data.frame(
#     targ_gene = row$targ_gene,
#     feat_gene = row$feat_gene,
#     D2 = D2_comb_cors,
#     D1_Ach = D1_Ach_cors,
#     D1_DRIVE = D1_DRIVE_cors
#   ))
# }) %>%
#   mutate(targ_gene = as.character(targ_gene),
#          feat_gene = as.character(feat_gene),
#          D1_best = pmax(D1_Ach, D1_DRIVE, na.rm=T),
#          targ_symbol = Gene_ID_to_symb[targ_gene],
#          is_Ach_CYCLOPS = targ_symbol %in% Tsherniak_CYCLOPS,
#          is_DRIVE_CYCLOPS = targ_symbol %in% DRIVE_CYCLOPS,
#          D2_imp = D2 - D1_best,
#          is_new = !is_Ach_CYCLOPS & !is_DRIVE_CYCLOPS,
#          new_type = ifelse(is_new, 'new', 'known'),
#          CE_type = ifelse(targ_gene %in% hart_ess, 'CE', 'non-CE'))
#
# ggplot(cur_res, aes(D1_best, D2)) +
#   geom_point(aes(shape = CE_type, color = new_type), size = 2, alpha = 0.7) +
#   geom_abline() +
#   geom_label_repel(data = cur_res %>%
#                     filter(is_new) %>%
#                     arrange(desc(D2_imp)) %>%
#                     head(10),
#                   aes(label = targ_symbol), color = 'red', size = 2., padding = 0.15) +
#   guides(shape = guide_legend(title = element_blank()),
#          color = guide_legend(title = element_blank())) +
#   theme_Publication() +
#   scale_colour_manual(values = c(new = 'red', known = 'black')) +
#   xlab('Best D1\nCYCLOPS correlation') +
#   ylab('D2-combined\nCYCLOPS correlation')
#
# export_fig('CYCLOPS_corr_compare', width = 3.5, height = 3.5)
#
# ##STATS
# D1_cyclops <- rel_an %>%
#   filter(dset %in% c('D1_Ach', 'D1_DRIVE'), MDP == 'Cyclops') %>%
#   .[['targ_gene']] %>%
#   unique()
# D2c_cyclops <- rel_an %>%
#   filter(dset %in% c('D2_comb'), MDP == 'Cyclops') %>%
#   .[['targ_gene']] %>%
#   unique()
# known_cyclops <- cur_res %>%
#   filter(is_Ach_CYCLOPS | is_DRIVE_CYCLOPS) %>%
#   .[['targ_gene']]
# print(sprintf('%d total CYCLOPS, %d unique to D2 combined, %d not known',
#               nrow(all_cyclops), length(setdiff(D2c_cyclops, D1_cyclops)), length(setdiff(D2c_cyclops, known_cyclops))))
#
# ggplot(rel_an %>%
#          filter(dset %in% c('D2_comb'),
#                 targ_gene %in% union(D1_cyclops, D2c_cyclops)) %>%
#           left_join(gene_avgs %>% filter(dset == 'D2_comb'),
#             by = c('targ_gene' = 'Gene')),
#        aes(avg_score, cor, color = targ_gene %in% D1_cyclops)) +
#   geom_point()
...
```{r}
rel_an %<>% mutate(target_partner = paste0(target, '_', partner),
                   targ_part_ID = paste0(targ_gene, '_', feat_gene)) %>%
            mutate(target_symbol = Gene_ID_to_symb[targ_gene],
                   feature_symbol = Gene_ID_to_symb[feat_gene])

cur_rel_an <- rel_an %>% 
  filter(dset %in% c('D2_comb', 'D1_Ach', 'D1_DRIVE')) %>%
  left_join(gene_avgs %>% filter(dset == 'D2_comb') %>% dplyr::select(-dset),
            by = c('targ_gene' = 'Gene'))

D2_unique_set <- setdiff(
  cur_rel_an %>% filter(dset == 'D2_comb') %>% .[['targ_part_ID']],
  cur_rel_an %>% filter(dset != 'D2_comb') %>% .[['targ_part_ID']]
)
D2_miss_set <- setdiff(
  cur_rel_an %>% filter(dset != 'D2_comb') %>% .[['targ_part_ID']],
...
ggplot(cur_rel_an %>% filter(dset == 'D2_comb'), 
       aes(avg_score, fill = targ_part_ID %in% D2_unique_set)) + 
  geom_density(alpha = 0.5) +
  guides(fill = F)

ggplot(cur_rel_an %>% filter(dset == 'D2_comb', MDP == 'paralog loss'), 
       aes(avg_score, cor, color = targ_part_ID %in% D2_unique_set)) + 
...
D2_unique <- rel_an %>%
  filter(dset == 'D2_comb') %>%
  distinct(targ_part_ID) %>%
  .[['targ_part_ID']] %>%
  setdiff(rel_an %>%
            filter(dset %in% c('D1_Ach', 'D1_DRIVE')) %>%
            distinct(targ_part_ID) %>%
            .[['targ_part_ID']])

D2_unique_rel_an <- rel_an %>%
  filter(!is.na(MDP)) %>%
  # filter(targ_part_ID %in% D2_unique, dset == 'D2_comb') %>%
  mutate(is_unique = targ_part_ID %in% D2_unique) %>% 
  filter(dset == 'D2_comb') %>%
  left_join(gene_avgs %>% filter(dset == 'D2_comb') %>% dplyr::select(-dset),
            by = c('targ_gene' = 'Gene'))

D2_unique_rel_an %<>% mutate(MDP = factor(MDP, levels = MDP_plot_order))
cur_cols <- hue_pal()(5) %>% set_names(MDP_plot_order)

D2_unique_rel_an %<>% mutate(target_partner = str_replace_all(target_partner, '_', '/'),
                             unique_status = ifelse(is_unique, 'D2-only', 'shared'))
ggplot(D2_unique_rel_an %>% filter(MDP %in% c('CYCLOPS', 'paralog loss')), 
         # filter(is_unique), 
       aes(avg_score, abs(cor), fill = factor(unique_status))) +
  # geom_point(alpha = 0.75) +
    geom_point(alpha = 1, shape = 21, size = 2, stroke = 0.25, color = 'white') +
  xlab('Cell line avg dependency') +
  ylab('Feature-dep corr\nmagnitude (D2 comb)') +
  theme_Publication() +
  # scale_fill_manual(values = cur_cols) +
  scale_size_manual(values = c(`FALSE` = 1.5, `TRUE` = 2.5)) +
  guides(fill = guide_legend(title = element_blank(), nrow=1)) +
  #        size = guide_legend(title = 'D2 unique', nrow = 2, override.aes = list(color = 'black',
  #                                                                               fill = 'black'))) +
  # geom_label_repel(data = D2_unique_rel_an %>%
  #                    arrange(desc(abs(cor))) %>%
  #                    head(10),
  #                  aes(label = target_partner), size = 2,
  #                  label.padding = 0.15, show.legend = F) +
  facet_wrap(~MDP)
export_fig('new_MDP_scatters', width = 5.5, height = 3.25)
```
...
all_pairs <- rel_an %>% filter(dset %in% c('D2_comb', 'D1_Ach', 'D2_DRIVE')) %>% 
  distinct(targ_part_ID, .keep_all=T) %>% 
  dplyr::select(targ_gene, feat_gene, feat_type) 

all_df_res <- adply(all_pairs, 1, function(row) {
  D1_Ach_cors <- get_spec_assoc('D1_Ach', row$targ_gene, row$feat_gene, row$feat_type, use_bayes = use_bayes)
  D1_DRIVE_cors <- get_spec_assoc('D1_DRIVE', row$targ_gene, row$feat_gene, row$feat_type, use_bayes = use_bayes)
  D2_comb_cors <- get_spec_assoc('D2_comb', row$targ_gene, row$feat_gene, row$feat_type, use_bayes = use_bayes)
  return(data.frame(
    targ_gene = row$targ_gene,
    feat_gene = row$feat_gene,
    D2 = D2_comb_cors,
  ...
  mutate(targ_gene = as.character(targ_gene),
         feat_gene = as.character(feat_gene),
         D1_DRIVE_mag = abs(D1_DRIVE),
         D1_Ach_mag = abs(D1_Ach),
         D1_best_mag = pmax(D1_Ach_mag, D1_DRIVE_mag, na.rm=T),
         D2_mag = abs(D2),
         targ_symbol = Gene_ID_to_symb[targ_gene],
         D2_imp = D2_mag - D1_best_mag,
         CE_type = ifelse(targ_gene %in% hart_ess, 'CE', 'non-CE'))
all_df_res %<>% left_join(gene_avgs %>% filter(dset == 'D2_comb') %>% dplyr::select(-dset), by = c('targ_gene' = 'Gene'))
all_df_res %<>% mutate(targ_part_ID = paste0(targ_gene, '_', feat_gene),
                       D2_unique = targ_part_ID %in% D2_unique
)
all_df_res %<>% left_join(rel_an %>% 
                            filter(targ_part_ID %in% all_df_res$targ_part_ID) %>% 
                            distinct(targ_part_ID, .keep_all=T) %>% 
                            dplyr::select(targ_part_ID, MDP),
                          by = 'targ_part_ID')
```
...
doMC::registerDoMC(cores = 5)
gdf <- related_genes %>%
  filter(source == 'paralog', target_ID %in% all_dep_genes) %>%
  dplyr::select(targ_gene = target_ID, feat_gene = partner_ID) %>%
  sample_n(size = 10000)

all_par_res <- adply(gdf, 1, function(row) {
  D1_Ach_cors <- get_best_assoc('D1_Ach', row$targ_gene, row$feat_gene, z_score = use_zscore, use_abs = use_abs)
  D1_DRIVE_cors <- get_best_assoc('D1_DRIVE', row$targ_gene, row$feat_gene, z_score = use_zscore, use_abs = use_abs)
  D2_comb_cors <- get_best_assoc('D2_comb', row$targ_gene, row$feat_gene, z_score = use_zscore, use_abs = use_abs)
  return(data.frame(
    targ_gene = row$targ_gene,
    feat_gene = row$feat_gene,
    D2 = D2_comb_cors,
  ...
  mutate(targ_gene = as.character(targ_gene),
         ...
         D1_best = pmax(D1_Ach, D1_DRIVE, na.rm=T),
         targ_symbol = Gene_ID_to_symb[targ_gene],
         D2_imp = abs(D2) - abs(D1_best),
         CE_type = ifelse(targ_gene %in% hart_ess, 'CE', 'non-CE'))
all_par_res %<>% left_join(gene_avgs %>% filter(dset == 'D2_comb'), by = c('targ_gene' = 'Gene'))
all_par_res %<>% filter(!is.infinite(D2))
...
use_abs <- FALSE
doMC::registerDoMC(cores = 5)
gdf <- data.frame(targ_gene = all_dep_genes,
                  feat_gene = all_dep_genes,
                  stringsAsFactors = F)
all_cyc_res <- adply(gdf, 1, function(row) {
  D1_Ach_cors <- get_best_assoc('D1_Ach', row$targ_gene, row$feat_gene, z_score = use_zscore, use_abs = use_abs)
  D1_DRIVE_cors <- get_best_assoc('D1_DRIVE', row$targ_gene, row$feat_gene, z_score = use_zscore, use_abs = use_abs)
  D2_comb_cors <- get_best_assoc('D2_comb', row$targ_gene, row$feat_gene, z_score = use_zscore, use_abs = use_abs)
  return(data.frame(
  ...
  mutate(targ_gene = as.character(targ_gene),
         ...
         new_type = ifelse(is_new, 'new', 'known'),
         CE_type = ifelse(targ_gene %in% hart_ess, 'CE', 'non-CE'))
# all_cyc_res %<>% mutate(gene_type = get_gene_type(targ_gene, hart_ess, hart_non_ess))
all_cyc_res %<>% left_join(gene_avgs %>% filter(dset == 'D2_comb'), by = c('targ_gene' = 'Gene'))
all_cyc_res %<>% filter(!is.infinite(D2))

tval <- 0.3
# ggplot(all_cyc_res, aes(D1_best, D2, color = avg_score )) + 
#   geom_point(alpha = 0.5, size = 1) + 
#   geom_abline() +
#   geom_vline(xintercept = tval, linetype = 'dashed') + 
#   geom_hline(yintercept = tval, linetype = 'dashed') +      
#   geom_vline(xintercept = -tval, linetype = 'dashed') +
#   geom_hline(yintercept = -tval, linetype = 'dashed') +
# scale_color_gradient(limits = c(-1, 0), oob = scales::squish,
#                      low = 'red', high = 'black') 

# plot_colorByDensity(all_cyc_res, 'D1_best', 'D2') +
all_cyc_res %<>% mutate(`Avg. dependency` = avg_score)
g <- ggplot(all_cyc_res, aes(D1_best, D2)) +
    geom_point(aes(fill = `Avg. dependency`), alpha = 0.75, shape = 21, size = 1.5, stroke = 0.25, color = 'white') +
  ...
  xlab('Gene dose-dependency\ncorr (D1 best)') +
  ylab('Gene dose-dependency\ncorr (D2 comb)') +
  scale_fill_gradient(low = 'red', high = 'black', limits = c(-1, 0), breaks = c(-1, 0), oob = scales::squish) +
  # guides(fill = guide_legend(title = 'Avg dependency')) +
  theme_Publication()
g1 <- g +   
  geom_label_repel(data = all_cyc_res %>%
                     filter(D2 < 0) %>% 
                    arrange(desc(D2_imp)) %>% 
                    head(5),
              aes(x = D1_best, y = D2, label = targ_symbol), size = 2, label.padding = 0.15)
export_fig('gene_dose_dep_all_scatter', width = 3.5, height = 3.5, plot = g1)

g2 <- g + coord_cartesian(xlim = c(0.1, 0.7), ylim = c(0.1, 0.75)) +
  ...
  geom_label_repel(data = all_cyc_res %>%
              ...
              aes(x = D1_best, y = D2, label = targ_symbol), size = 2, label.padding = 0.15) +
  guides(fill = F)
export_fig('gene_dose_dep_all_scatter_zoom', width = 2.5, height = 2.5, plot = g2)

with(all_cyc_res,
     table(D2 = D2 > tval, known = is_Ach_CYCLOPS | is_DRIVE_CYCLOPS))

with(all_cyc_res,
     table(D2 = D2 > tval, D1 = D1_best > tval))

# all_cyc_res %<>% mutate(new = ifelse(is_Ach_CYCLOPS | is_DRIVE_CYCLOPS, 'known-CYCLOPS', 'other'))
all_cyc_res %<>% mutate(targ_part_ID = paste0(targ_gene, '_', feat_gene),
                        new = ifelse(targ_part_ID %in% D2_unique_set, 'known-CYCLOPS', 'other'))
ggplot(all_cyc_res, aes(avg_score, D2)) +
      geom_point(alpha = 0.6, shape = 21, size = 1.5, stroke = 0.25, color = 'white', 
                 aes(fill = new)) +
# geom_point(alpha = 0.5, size = 1.5, aes(color = is_Ach_CYCLOPS | is_DRIVE_CYCLOPS)) +
  ...
  guides(color = F) +
  # geom_text_repel(data = all_cyc_res %>% 
  #                   arrange(desc(D2)) %>% 
  #                   head(20),
  #                 aes(label = targ_symbol), size = 2.5) +
  coord_cartesian(ylim = c(-0.25, 0.7), xlim = c(-1.6, 0.25)) +
  scale_fill_manual(values = c(`known-CYCLOPS` = 'red', `other` = 'black')) +
  xlab('Cell line avg dependency') +
  ylab('Gene dose-dependency\ncorr (D2 comb)') +
  theme_Publication() +
  guides(fill = guide_legend(title = ''))
    # scale_alpha_manual(values = c(`FALSE` = 0.25, `TRUE` = 1))
export_fig('gene_dose_dep_ess_scatter', width = 3.5, height = 3.5)
...
#S8
##S8a: Uncertainty estimates capture cell line quality differences
...
Ach_rep_cors <- read.csv('~/CPDS/demeter2/results/Ach_rep_cors.csv', stringsAsFactors = F, check.names = F)
unc_df <- per_CL_stats %>% 
    filter(dset == 'GA_Ach') %>% 
...
ggsave(file.path(fig_dir, 'SSMD_vs_avg_unc.png'), width = 3.5, height = 3, dpi = 350)
```
...
ggsave(file.path(fig_dir, 'repcor_vs_avg_unc.png'), width = 3.5, height = 3, dpi = 350)

```


## S8c: Uncertainty estimates reflect number of good hairpins per gene
...
ggsave(file.path(fig_dir, 'hpEff_vs_avg_unc.png'), width = 3.5, height = 3, dpi = 350)
```


## S8d: Accounting for variable uncertainty improves agreement with CRISPR
...
ggsave(file.path(fig_dir, 'CRISPR_cors_unc_smooth.png'), plot = p, width = 6.5, height = 3, dpi = 350)

##STATS
print('Achilles')
print_paired_two_group_stats(CRISPR_cors_unc_spread$D2noUnc_Ach,
                             CRISPR_cors_unc_spread$D2_Ach)

print('DRIVE')
print_paired_two_group_stats(CRISPR_cors_unc_spread$D2noUnc_DRIVE,
                             CRISPR_cors_unc_spread$D2_DRIVE)
```

#S3
## S3a: RNAi efficacy is closely related with screen quality
...
ggsave(file.path(fig_dir, 'PR_vs_RNAiEff.png'), width = 3.5, height = 3, dpi = 350)
...
ggplot(df, aes(ssmd)) + 
    ...
    scale_fill_manual(values = data_type_pal) +
    theme_Publication()
ggsave(file.path(fig_dir, 'SSMD_vs_RNAiEff.png'), width = 3.5, height = 3, dpi = 350)
...
## S3b: Agreement between Achilles and DRIVE RNAi efficacy estimates

```{r, fig.width=5, fig.height=4}
DRIVE_CL_data <- read.csv(file.path(dep_datasets$D2_DRIVE$path, 'CL_data.csv'), stringsAsFactors = F) 
Ach_CL_data <-read.csv(file.path(dep_datasets$D2_Ach$path, 'CL_data.csv'), stringsAsFactors = F)

Ach_CL_batch_data <-read.csv(file.path(dep_datasets$D2_Ach$path, 'CL_batch_data.csv'), stringsAsFactors = F) 
DRIVE_CL_batch_data <-read.csv(file.path(dep_datasets$D2_DRIVE$path, 'CL_batch_data.csv'), stringsAsFactors = F) 

#compute average overall slope per CL
...
ggsave(file.path(fig_dir, 'RNAi_eff_compare.png'), width = 3.5, height = 3, dpi = 350)
...
# S11: Cross-validated estimates of pos-neg control separation
...
test_SSMD %>% melt() %>% 
    ...
    geom_jitter(width = 0.0, alpha = 0.75, cex = 2) +
    ylab('Pos-neg control separation (SSMD)') +
    theme_Publication() +
    theme(axis.title.x = element_blank(),
          axis.text.x = element_text(size = 12))

ggsave(file.path(fig_dir, 'SSMD_xval.png'), width = 3, height = 3, dpi = 350)

```


## S9a: PCs of merged datasets
```{r}
merge_sets <- list(
  RSA = list(Ach = 'RSA_Ach', DRIVE = 'RSA_DRIVE'),
   ATARiS = list(Ach = 'ATARiS_Ach', DRIVE = 'ATARiS_DRIVE'),
   D1 = list(Ach = 'D1_Ach', DRIVE = 'D1_DRIVE'),
   MAGeCK = list(Ach = 'MAGeCK_Ach', DRIVE = 'MAGeCK_DRIVE')
  )

#make Ach-DRIVE merged datasets
center <- TRUE
all_merge_res <- llply(names(merge_sets) %>% set_names(names(merge_sets)), function(mname) {
    # MAKE manual merge of 3 datasets
  cur_merge <- make.a.tidy.dataset(
      list(Ach = dep_data[[merge_sets[[mname]]$Ach]]$gene_scores,
           DRIVE = dep_data[[merge_sets[[mname]]$DRIVE]]$gene_scores),
      na.cols = TRUE, na.rows = TRUE, dim.names = c('CCLE_ID', 'Gene')
  )
  cur_merge$Avg <- rowMeans(cur_merge[, c('Ach', 'DRIVE')], na.rm=T)
  cur_merge %<>%
      dplyr::select(CCLE_ID, Gene, Avg) %>%
      spread(Gene, Avg)
  cur_merge %<>% remove_rownames() %>% column_to_rownames(var = 'CCLE_ID') %>% 
    as.matrix()
  
  #compute top 2 PCs
  cur_merge[is.na(cur_merge)] <- NA
  cur_pcs <- pcaMethods::pca(cur_merge,
                            method = 'ppca',
                            nPcs = 2,
                            scale = 'none',
                            center = center)
  cur_R2sum <- R2cum(cur_pcs)
  cur_pcs <- scores(cur_pcs)
  
  cur_df <- data.frame(
        CCLE_ID = rownames(cur_merge), 
                     PC1 = cur_pcs[,1], 
                     PC2 = cur_pcs[,2]  
      ) %>% mutate(model = mname)
  return(list(PC_scores = cur_df, PC_vars = diff(c(0, cur_R2sum))))
})
get_cl_memb <- function(x) {
    m <- rep(NA, length(x))
    m[x %in% Ach_CLs] <- Ach_batch[x[x %in% Ach_CLs]]
...
all_PCs <- ldply(all_merge_res, function(x) x$PC_scores) 
all_PCs %<>% ddply(.(model), function(df) {
    df %<>% mutate(CL_m = get_cl_memb(as.character(CCLE_ID)))
})
...
PC_vars <- all_merge_res$RSA$PC_vars*100
ggplot(all_PCs %>% filter(model == 'RSA'), 
       aes(PC1, PC2, fill = CL_m)) + 
    geom_point(shape = 21, size = 2, color = 'white', alpha = 0.6) + 
    guides(fill = guide_legend(title = 'Batch', nrow = 2)) +
    xlab(sprintf('PC1 (%.1f%% variance)', PC_vars[1])) + 
    ylab(sprintf('PC2 (%.1f%% variance)', PC_vars[2])) + 
    ggtitle('RSA-combined') +
    scale_fill_manual(values = cmap) +
    theme_Publication(base_font) 
export_fig('RSA_combined_PCs', 3.5, 3.5)

PC_vars <- all_merge_res$MAGeCK$PC_vars*100
ggplot(all_PCs %>% filter(model == 'MAGeCK'), 
       aes(PC1, PC2, fill = CL_m)) + 
    geom_point(shape = 21, size = 2, color = 'white', alpha = 0.6) + 
    guides(fill = guide_legend(title = 'Batch', nrow = 2)) +
    xlab(sprintf('PC1 (%.1f%% variance)', PC_vars[1])) + 
    ylab(sprintf('PC2 (%.1f%% variance)', PC_vars[2])) + 
    ggtitle('MAGeCK-combined') +
    scale_fill_manual(values = cmap) +
    theme_Publication(base_font) 
export_fig('MAGeCK_combined_PCs', 3.5, 3.5)


PC_vars <- all_merge_res$D1$PC_vars*100
ggplot(all_PCs %>% filter(model == 'D1'), 
       aes(PC1, PC2, fill = CL_m)) + 
    geom_point(shape = 21, size = 2, color = 'white', alpha = 0.6) + 
    guides(fill = guide_legend(title = 'Batch', nrow = 2)) +
    xlab(sprintf('PC1 (%.1f%% variance)', PC_vars[1])) + 
    ...
    scale_fill_manual(values = cmap) +
    theme_Publication(base_font) 
export_fig('D1_combined_PCs', 3.5, 3.5)

PC_vars <- all_merge_res$ATARiS$PC_vars*100
ggplot(all_PCs %>% filter(model == 'ATARiS'), 
       aes(PC1, PC2, fill = CL_m)) + 
    geom_point(shape = 21, size = 2, color = 'white', alpha = 0.6) + 
    guides(fill = guide_legend(title = 'Batch', nrow = 2)) +
    xlab(sprintf('PC1 (%.1f%% variance)', PC_vars[1])) + 
    ylab(sprintf('PC2 (%.1f%% variance)', PC_vars[2])) + 
    ggtitle('ATARiS-combined') +
    scale_fill_manual(values = cmap) +
    theme_Publication(base_font) 
export_fig('ATARiS_combined_PCs', 3.5, 3.5)

```


## S10 Scale param v doubling time

```{r}
Ach_CL_data <-read.csv(file.path(dep_datasets$D2_Ach$path, 'CL_data.csv'), stringsAsFactors = F) %>% 
    left_join(read.csv(file.path(dep_datasets$D2_Ach$path, 'CL_batch_data.csv'), stringsAsFactors = F) %>% 
    group_by(CCLE_ID) %>% 
...
p1 <- ggplot(df, aes(ov_slope, Doubling_time)) + 
    geom_point(alpha = 0.75) + 
    scale_y_log10() +
    scale_x_log10(breaks = c(0.25, 0.5, 1.0, 2.0)) +
...
p2 <- ggplot(df, aes(gene_slope, Doubling_time)) + 
    geom_point(alpha = 0.75) + 
    scale_y_log10() +
    scale_x_log10(breaks = c(0.25, 0.5, 1.0, 2, 4)) +
    geom_smooth(method = 'lm') +
    ylab('Doubling time (hrs)') +
...
ggsave(file.path(fig_dir, 'doubling_time_scatter.png'), width = 6, height = 6, dpi = 350)

```

#S2
## S2a: LFC SSMD compare

```{r}
#get shRNA seqs for targeting pos and neg cons
...
BGPD.LFC.mat <- load.from.taiga(data.name='drive-lfc-matrices-3867', data.version=12, data.file='BGPD_LFC_mat')
poolA.LFC.mat <- load.from.taiga(data.name='drive-lfc-matrices-3867', data.version=12, data.file='poolA_LFC_mat')
poolB.LFC.mat <- load.from.taiga(data.name='drive-lfc-matrices-3867', data.version=12, data.file='poolB_LFC_mat')
...
#z-score normalize and join melted data
DRIVE_df <- bind_rows(
    list(
        BGPD.LFC.mat %>% 
...
ggplot(hp_ssmds, aes(Achilles, DRIVE)) + 
    geom_point(alpha = 0.75) + 
    geom_abline() +
    xlab('Achilles pos/neg\nseparation (SSMD)') +
    ylab('DRIVE pos/neg\nseparation (SSMD)') +
    theme_Publication() + 
    ggtitle('shRNA')
ggsave(file.path(fig_dir, 'LFC_SSMD_compare.png'), width = 3.75, height = 3.25, dpi = 350)

```

##S2c: SSMD for common vs unique shRNA sets
...
ggplot(hp_ssmd_t, aes(SSMD_diff, group = type, color = type)) + 
  ...
  ylab('Num cell lines') +
  theme_Publication()
ggsave(file.path(fig_dir, 'LFC_SSMD_diff_hist.png'), width = 3.75, height = 3.25, dpi = 350)
...
ggplot(temp_per_CL_stats, aes(GA_Ach, GA_DRIVE)) + 
    ...
    theme_Publication() +
    ggtitle('Gene')
ggsave(file.path(fig_dir, 'GA_SSMD_compare.png'), width = 3.5, height = 3.25, dpi = 350)
...
ggplot(temp, aes(gene_type, avg_score, fill = data_set)) + 
    ...
    theme_Publication() +
    theme(axis.title.x = element_blank())
ggsave(file.path(fig_dir, 'Ach_DRIVE_geneavg_compare.png'), width = 3.5, height = 3, dpi = 350)

```


## S2e: Ach_DRIVE bench cor compare
...
ggplot(bench_cors) + 
    ...
    xlim(0, mp) + ylim(0, mp) +
    theme_Publication()
ggsave(file.path(fig_dir, 'Ach_DRIVE_bench_compare.png'), width = 3.75, height = 3.25, dpi = 350)

print('D2 Ach vs DRIVE')
print_paired_two_group_stats(abs(bench_cors$D2_Ach), abs(bench_cors$D2_DRIVE))

print('GA Ach vs DRIVE')
print_paired_two_group_stats(abs(bench_cors$GA_Ach), abs(bench_cors$GA_DRIVE))

```

#S5
## S5a: Normalizing by pos/neg controls per cell line amplifies noise
...
#compute gene avgs for these datasets
...
## PCA projection of D1

```{r}
#project out first PC of D1 data
center <- TRUE
nPcs <- 20 #set to 20 here because using PCs to impute missing values
D1_pcs <- pcaMethods::pca(dep_data$D1_Ach$gene_scores,
                          method = 'ppca',
                          nPcs = nPcs,
                          scale = 'none',
                          center = center)

imp_mat <- pcaMethods::completeObs(D1_pcs) #impute misisng values based on PCs
first_PC_D1_Ach <- pcaMethods::scores(D1_pcs)[,1] #first PC
first_PC_D1_Ach <- matrix(first_PC_D1_Ach/sqrt(sum(first_PC_D1_Ach^2, na.rm=T)), ncol = 1) #norm by L2-mag

#project first PC out of matrix
proj_mat <- first_PC_D1_Ach %*% t(first_PC_D1_Ach)
proj_D1_data <- imp_mat - proj_mat %*% imp_mat
rownames(proj_D1_data) <- rownames(dep_data$D1_Ach$gene_scores)
colnames(proj_D1_data) <- colnames(dep_data$D1_Ach$gene_scores)

#create this PC-projected D1 matrix
dep_data$D1PC_Ach <- list(gene_scores = proj_D1_data, 
                            mod_type = 'D1',
                            dataset = 'Ach', 
                            type = 'rel')

#Same thing for DRIVE
D1_DRIVE_pcs <- pcaMethods::pca(dep_data$D1_DRIVE$gene_scores,
                          method = 'ppca',
                          nPcs = nPcs,
                          scale = 'none',
                          center = center)

imp_mat <- pcaMethods::completeObs(D1_DRIVE_pcs)
first_PC_D1_DRIVE <- pcaMethods::scores(D1_DRIVE_pcs)[,1] #first PC
first_PC_D1_DRIVE <- matrix(first_PC_D1_DRIVE/sqrt(sum(first_PC_D1_DRIVE^2, na.rm=T)), ncol = 1)
proj_mat <- first_PC_D1_DRIVE %*% t(first_PC_D1_DRIVE)
proj_D1_data <- imp_mat - proj_mat %*% imp_mat
rownames(proj_D1_data) <- rownames(dep_data$D1_DRIVE$gene_scores)
colnames(proj_D1_data) <- colnames(dep_data$D1_DRIVE$gene_scores)

dep_data$D1PC_DRIVE <- list(gene_scores = proj_D1_data, 
                            mod_type = 'D1',
                            dataset = 'DRIVE', 
                            type = 'rel')
```

## S5b: Compare PC1 with screen signal

```{r}
DRIVE_CL_data <- read.csv(file.path(dep_datasets$D2_DRIVE$path, 'CL_data.csv'), stringsAsFactors = F) %>% 
  left_join(data.frame(
    CCLE_ID = rownames(dep_data$D1_DRIVE$gene_scores),
    PC = first_PC_D1_DRIVE[,1]
  ), by = 'CCLE_ID')

Ach_CL_data <-read.csv(file.path(dep_datasets$D2_Ach$path, 'CL_data.csv'), stringsAsFactors = F) %>% 
  left_join(data.frame(
    CCLE_ID = rownames(dep_data$D1_Ach$gene_scores),
    PC = first_PC_D1_Ach[,1]
  ), by = 'CCLE_ID')

g1 <- ggplot(Ach_CL_data, aes(gene_slope, PC)) + 
   geom_point(alpha = 0.75, shape = 21, size = 1.5, color = 'white', fill = 'black') +
  xlab('Screen signal') + ylab('First PC loading') +
  theme_Publication() +
  ggtitle('Achilles')
g2 <- ggplot(DRIVE_CL_data, aes(gene_slope, PC)) + 
geom_point(alpha = 0.75, shape = 21, size = 1.5, color = 'white', fill = 'black') +
  xlab('Screen signal') + ylab('First PC loading') +
  theme_Publication() +
  ggtitle('DRIVE')
...
export_fig('PC_screen_signal_compare', width = 7, height = 3.5, plot = p)

```


##S5c: Compare AG02 corrs for other norm methods

```{r}
vector <- feature_data$GE[, '27161'] %>% set_names(rownames(feature_data$GE))

target_deps <- c('D1_Ach', 'D1PC_Ach', 'D1_DRIVE', 'D1PC_DRIVE', 'GA_Ach', 'GAPN_Ach', 'GA_DRIVE', 'GAPN_DRIVE','D2_Ach', 'D2_DRIVE')
...
AGO2_cors %<>% mutate(gene_type = plyr::revalue(gene_type, c(non_essential = 'non-essential'))) %>% mutate(mod = str_match(model, '([:alnum:]+)_[:alnum:]+')[,2],
                      dat = str_match(model, '[:alnum:]+_([:alnum:]+)')[,2],
                      mod = factor(mod, levels = c('D2', 'D1', 'D1PC', 'GA', 'GAPN')))

AGO2_cors %<>% mutate(mod = plyr::revalue(mod, c(D1PC = 'D1-PC',
                                          GAPN = 'GA-indnorm')))
...
export_fig('norm_methods_AGO2_corrs', width = 7, height = 3.5, plot = p)

```

##S5d: Benchmark feature-dep corrs for other norm methods

```{r}
target_dsets <- c('D2_Ach', 'D2_DRIVE', 'D1_Ach', 'D1PC_Ach', 'D1_DRIVE', 'D1PC_DRIVE', 'GA_Ach', 'GAPN_Ach', 'GA_DRIVE', 'GAPN_DRIVE')

cur_gene_set <- NULL
cur_CL_set <- NULL
bench_res_test <- get_target_biomarker_dep_cors(target_dsets, CRISPR_benchmarks, feature_data, cur_gene_set, cur_CL_set, use_bayes = use_bayes) %>% 
  mutate(mod = str_match(model, '(.+)_')[,2],
         mod = plyr::revalue(mod, c(D1PC = 'D1-PC',
                                          GAPN = 'GA-indnorm')))

p1 <- ggplot(bench_res_test %>% filter(grepl('Ach', model)),
       aes(abs(cor), color = mod)) + 
    stat_ecdf(size = 0.6) +
    scale_x_reverse() +
...
p2 <- ggplot(bench_res_test %>% filter(grepl('DRIVE', model)),
       aes(abs(cor), color = mod)) + 
    stat_ecdf(size = 0.6) +
    scale_x_reverse() +
    xlab('Correlation magnitude |r|') +
...
export_fig('norm_methods_CRISPR_benchcors', width = 6, height = 3.5, plot = p)

bench_res_test_spread <- spread(bench_res_test %>% dplyr::select(-p.value, -ptype, -mod) %>% mutate(cor = abs(cor)), model, cor)

compare_bench_results <- function(bench_res_spread, type1, type2) {
  print(sprintf('Comparing bench results for %s vs %s', type1, type2))
  print(sprintf("Wilcox p: %.3g", wilcox.test(bench_res_spread[[type1]], bench_res_spread[[type2]], paired = TRUE)$p.value))
  print(sprintf("Med diff p: %.4f", median(bench_res_spread[[type1]] - bench_res_spread[[type2]], na.rm=T)))
  print(sprintf("N shared p: %d", sum(!is.na(bench_res_spread[[type1]]) & !is.na(bench_res_spread[[type2]]))))
}

compare_bench_results(bench_res_test_spread, 'GA_Ach', 'GAPN_Ach')
compare_bench_results(bench_res_test_spread, 'GA_DRIVE', 'GAPN_DRIVE')
compare_bench_results(bench_res_test_spread, 'D1_Ach', 'D1PC_Ach')
compare_bench_results(bench_res_test_spread, 'D1_DRIVE', 'D1PC_DRIVE')
