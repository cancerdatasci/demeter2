...
fig_dir <- '~/CPDS/demeter2/results/final_figures'
...
export_fig <- function(fig_name, width, height, plot = NULL) {
    if (is.null(plot)) {
        plot <- last_plot()
    }
...
## 1D: Per-gene avg agreement across datasets
...
print(sprintf('Ach-DRIVE MAGeCK: %.3f', df[df$dset == 'MAGeCK Ach DRIVE', 'cor']))
...
export_fig('ind_example_MA_compare', width = 5, height = 7, plot = p)
```
...
ggplot(df, aes(AGO2_GE)) + 
    ...
    geom_point(alpha = 0.75, shape = 21, size = 1.5, stroke = 0.25, color = 'white', aes(y = Ach_gene_slope, fill = 'Achilles')) +
    geom_point(alpha = 0.75, shape = 21, size = 1.5, stroke = 0.25, color = 'white', aes(y = DRIVE_gene_slope, fill = 'DRIVE')) +
    ...
    theme_Publication(base_font)
export_fig('AGO2_vs_RNAi_eff', width = 4, height = 3.5)

ggplot(df, aes(AGO2_GE)) + 
    ylab('Screen quality\n(pos/neg control SSMD)') +
    xlab('AGO2 Gene expression (log RPKM)') +
    geom_point(alpha = 0.75, shape = 21, size = 1.5, stroke = 0.25, color = 'white', aes(y = Ach_GA_ssmd, fill = 'Achilles')) +
    geom_point(alpha = 0.75, shape = 21, size = 1.5, stroke = 0.25, color = 'white', aes(y = DRIVE_GA_ssmd, fill = 'DRIVE')) +
    ...
    scale_fill_manual(values = data_type_pal) +
    theme_Publication(base_font)
export_fig('AGO2_vs_SSMD', width = 3.6, height = 3.25)
...
export_fig('AGO2_corrs_scatter', width = 6.5, height = 3.5, plot = p)
...
export_fig('AGO2_corrs_smooth', width = 7, height = 3.5, plot = p)
...
export_fig('avg_depcor', width = 8, height = 3.5, plot = p)
print(p)
```
...
p <- grid.arrange(arrangeGrob(p1 + theme(legend.position="none"),
                         ...
                         nrow=1),
             mylegend, nrow=2,heights=c(10, 1))
export_fig('avg_depcor_all_smoothed', width = 8, height = 3.5, plot = p)
```
...
tiff(file.path(fig_dir, 'D2_cornet_examp.tiff'), width = 5, height = 6, units = 'in', res = 350)
make_net_plot(df %>% mutate(cor = D2_c, cor_z = D2_c_z), c_D2_z)
dev.off()

tiff(file.path(fig_dir, 'D1_cornet_examp.tiff'), width = 5, height = 6, units = 'in', res = 350)
make_net_plot(df %>% mutate(cor = D1_c, cor_z = D1_c_z), c_D1_z)
dev.off()
...
export_fig('CRISPR_cor_smooth', width = 6.5, height = 3.5, plot = p)
export_fig('CRISPR_cor_smooth_Ach', width = 3.5, height = 3, plot = p1)
export_fig('CRISPR_cor_smooth_DRIVE', width = 3.5, height = 3, plot = p2)
...
export_fig('GE_cor_smooth', width = 6, height = 3.5, plot = p)
export_fig('GE_cor_smooth_Ach', width = 3.5, height = 3, plot = p1)
export_fig('GE_cor_smooth_DRIVE', width = 3.5, height = 3, plot = p2)
```
...
export_fig('CN_cor_smooth', width = 6, height = 3.5, plot = p)
export_fig('CN_cor_smooth_Ach', width = 3.5, height = 3, plot = p1)
export_fig('CN_cor_smooth_DRIVE', width = 3.5, height = 3, plot = p2)
```
...
export_fig('cyclops_example', width = 6, height = 3.5, plot = p)
...
export_fig('bench_ecdf', width = 6.5, height = 3.5, plot = p)
export_fig('bench_ecdf_Ach', width = 3.5, height = 3, plot = p1)
export_fig('bench_ecdf_DRIVE', width = 3.5, height = 3, plot = p2)
```
...
p <- arrangeGrob(p1 + theme(legend.position="none"),
                         p2 + theme(legend.position="none"),
                         nrow=1)
export_fig('bench_strong_cors_bar', width = 6.5, height = 3.5, plot = p)
export_fig('bench_strong_cors_bar_Ach', width = 3.5, height = 3, plot = p1)
export_fig('bench_strong_cors_bar_DRIVE', width = 3.5, height = 3, plot = p2)
```
...
p <- arrangeGrob(p2 + theme(legend.position="none"),
                         ...
                         nrow=1)
export_fig('PCA_compare', width = 6, height = 3, plot = p)
```
...
export_fig('combined_Gavg_CRISPR_cor_compare', width = 3.5, height = 3, plot = p2)
...
export_fig('combined_CRISPR_corr_smooth', width = 3.5, height = 3)
...
export_fig('combined_bench_ecdf', width = 3.5, height = 3)
```
...
export_fig('bench_strong_cors_comb_bar', width = 3, height = 2.5)
```
...
ggplot(df %>% filter(dset %in% c('D2_Ach', 'D2_DRIVE', 'D2_comb', 'GA_comb', 'D1_merge')) %>% 
    ...
    scale_color_manual(values = data_type_pal) +
    theme_Publication(base_font) 
export_fig('combined_bench_qvalues', width = 3.75, height = 3.5)
...
export_fig('combined_N_biomarks_found', width = 3.5, height = 3.5)
```
...
export_fig('combined_top_MDP_class', width = 3.5, height = 3.5)
...
## 5f: Dose-dependency corr analysis
...
#compute dose-dependency correlations across all genes 
...
all_cyc_res %<>% left_join(gene_avgs %>% filter(dset == 'D2_comb'), by = c('targ_gene' = 'Gene')) %>% filter(!is.infinite(D2))
...
export_fig('SSMD_vs_avg_unc', width = 3.5, height = 3)
```
...
export_fig('repcor_vs_avg_unc', width = 3.5, height = 3)
```
...
export_fig('hpEff_vs_avg_unc', width = 3.5, height = 3)
...
export_fig('CRISPR_cors_unc_smooth', width = 6.5, height = 3, plot = p)
...
export_fig('PR_vs_RNAiEff', width = 3.5, height = 3)
...
ggplot(df, aes(ssmd)) + 
    ...
    scale_fill_manual(values = data_type_pal) +
    theme_Publication()
export_fig('SSMD_vs_RNAiEff', width = 3.5, height = 3)
...
export_fig('RNAi_eff_compare', width = 3.5, height = 3)
...
export_fig('SSMD_xval', width = 3.5, height = 3)
```
...
export_fig('doubling_time_scatter', width = 6, height = 6)
```
...
ggplot(hp_ssmds, aes(Achilles, DRIVE)) + 
    ...
    theme_Publication() + 
    ggtitle('shRNA')
export_fig('LFC_SSMD_compare', width = 3.75, height = 3.25)
...
ggplot(hp_ssmd_t, aes(SSMD_diff, group = type, color = type)) + 
  ...
  ylab('Num cell lines') +
  theme_Publication()
export_fig('LFC_SSMD_diff_hist', width = 3.75, height = 3.25)
...
ggplot(temp_per_CL_stats, aes(GA_Ach, GA_DRIVE)) + 
    ...
    theme_Publication() +
    ggtitle('Gene')
export_fig('GA_SSMD_compare', width = 3.5, height = 3.25)
...
export_fig('Ach_DRIVE_geneavg_compare', width = 3.5, height = 3)
```
...
ggplot(bench_cors) + 
    ...
    xlim(0, mp) + ylim(0, mp) +
    theme_Publication()
export_fig('Ach_DRIVE_bench_compare', width = 3.75, height = 3.25)
...
p <- grid.arrange(arrangeGrob(p1 + theme(legend.position="none"),
                         ...
                         nrow=2),
             mylegend, nrow=2,heights=c(18, 1))
export_fig('GAnorm_MA_compare', width = 5, height = 7, plot = p)
...
p1 <- ggplot(bench_res_test %>% filter(grepl('Ach', model)),
    ...
    ylab('CDF') +
    coord_cartesian(xlim = c(0.6, 0), ylim = c(0, 1)) +
    # xlim(0.6, 0) +
    # ylim(0, 0.75) +
...
p2 <- ggplot(bench_res_test %>% filter(grepl('DRIVE', model)),
    ...
    xlab('Correlation magnitude |r|') +
    ylab('CDF') +
    coord_cartesian(xlim = c(0.75, 0), ylim = c(0, 1)) +
    # xlim(0.6, 0) +
    # ylim(0, 0.75) +
    guides(color = guide_legend(title = element_blank(), nrow = 2, keyheight = 1.15, override.aes = list(size = 3))) + 
    ggtitle('DRIVE') + 
...
compare_bench_results <- function(bench_res_spread, type1, type2) {
  ...
  print(sprintf("T-test p: %.3g", t.test(bench_res_spread[[type1]], bench_res_spread[[type2]], paired = TRUE)$p.value))
  print(sprintf("Med diff p: %.4f", median(bench_res_spread[[type1]] - bench_res_spread[[type2]], na.rm=T)))
  print(sprintf("Mean diff p: %.4f", mean(bench_res_spread[[type1]] - bench_res_spread[[type2]], na.rm=T)))
