...
CERES = load.from.taiga(
             data.name = 'avana-1-2-8b72',
             data.version = 2, 
             data.file = 'ceres_gene_effects',
             transpose = T)
new_ceres <- load.from.taiga(data.name='avana-564b', data.version=14, data.file='gene_effect')
...
colnames(CERES) <- ceres_gmap[colnames(CERES)]
new_ceres <- NULL #clear from mem


feature_datasets <- list(
    GE = list(taiga_name = 'rnaseq-ccle-rsem-tpm-entrez-8ddd',
              taiga_version = 1),
    CN = list(taiga_name = 'gene-level-cn-87aa', taiga_version = 3, 
